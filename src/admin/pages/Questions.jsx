...
import { BookOpen, HelpCircle, PlusCircle, FileText } from "lucide-react";
...
const AddContentPage = () => {
  ...
  const tabs = [
    ...
    { name: "Grammar", value: "grammar", icon: <FileText className="mr-2" /> },
  ];
  ...
  const TabContent = () => {
    ...
    if (selectedTab === "grammar") {
      return (
        <form className="space-y-4">
          <div>
            <label htmlFor="rule" className="block text-sm font-medium">
              Grammar Rule
            </label>
            <textarea
              id="rule"
              className="w-full border rounded-lg p-2 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300"
              placeholder="Enter the grammar rule"
            ></textarea>
          </div>
          <div>
            ...
            <textarea
              id="examples"
              className="w-full border rounded-lg p-2 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300"
              placeholder="Provide example sentences"
            ></textarea>
          </div>
        </form>
      );
  ...
  return (
    <div className="p-2 sm:p-6 bg-gray-100 dark:bg-slate-950 min-h-screen">
      ...
      <div className="flex flex-wrap justify-around sm:justify-center gap-4 mb-6 border-b pb-3 dark:border-slate-700">
  {tabs.map((tab) => (
    <button
      key={tab.value}
