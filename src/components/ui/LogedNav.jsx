...
const LogedNav = () => {
  ...
  return (
    <nav className="shadow-sm border-b-default border-solid border-gray-200 z-10 w-full fixed bg-white">
      <div aria-label="top bar" className=" px-4 sm:px-8 flex-none flex justify-between bg-white h-16">

        {/* <!-- top bar left --> */}
        <div className="flex items-center justify-center">
            <Link to={'/'} className="font-bold text-2xl mb-1">Ai Tutor</Link>
            <Menu onClick={handleMenu} className=" ml-2 w-6 h-6 text-gray-700 hover:text-black lg:hidden cursor-pointer" />
            <ul aria-label="top bar left" aria-orientation="horizontal" className="hidden sm:flex">
                    {/* <!-- add button --> */}
        ...
        {/* menu */}
        {showMenu && (<div className='absolute sm:w-3/6 w-full bg-slate-300 top-16 left-0 h-full '><SideMenu /></div>)}

        {/* <!-- to bar right  --> */}
        <ul aria-label="top bar right" aria-orientation="horizontal" className=" flex items-center">
            <li className="relative">
                <input title="Search Bar" aria-label="search bar" role="search" className="pr-8 pl-4 py-2 rounded-md cursor-pointer transition-all duration-300 ease-in-out focus:border-black focus:cursor-text w-4 focus:w-40 ml-2 sm:ml-0 sm:focus:w-64 placeholder-transparent focus:placeholder-gray-500" type="text" placeholder="Search Chi Desk Support" />
            ...
            <li className="h-8 w-8 ml-1">
                <button title="Notifications" aria-label="notifications" className="w-full h-full text-slate-800">
                    <i>
                        <svg className="fill-current w-4 h-4 mx-auto" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
